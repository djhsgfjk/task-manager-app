	...
	onCardsDragEnd = (result) => {
		...
		if (pastListId === newListId) {
			if (!(pastIndex === newIndex)) {
				...
				this.props.updateListAndCards({projectId: projectId, list: {...list, cards: cards.map((card, index) => ({...card, index: index}))}})
			...
			this.props.updateListAndCards({projectId: projectId, list: {
				...newList,
						...
						<Droppable droppableId={'all-lists'} direction={'horizontal'} type='lists'>
							{(provided) => (
								...
								>
									...
									<ActionButton list userId={user.id} projectId={currentProject.id} index={newListIndex} />
								</div>
							)}
						</Droppable>
					</DragDropContext>
					</div>
					{modal.modalOn ? <UpdateCard modalOn={modal.modalOn} projectId={currentProject.id} card={modal.card}/> : null}
